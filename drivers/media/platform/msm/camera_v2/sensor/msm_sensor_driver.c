	...
	struct msm_sensor_power_setting *pu)
{
	...
	for (c = 0; c < size_down; c++) {
		if(pd[c].seq_type == SENSOR_GPIO){
			if(pd[c].config_val == GPIO_OUT_HIGH)
				pd[c].config_val = GPIO_OUT_LOW;
			else if(pd[c].config_val == GPIO_OUT_LOW)
				pd[c].config_val = GPIO_OUT_HIGH;
		}
	}
	return rc;
}
...
/* LGE_CHANGE_S, apply proper power setting, 2014-10-29, yt.jeon@lge.com */
	enum hw_rev_type rev_type = 0;
	pr_info("%s:%d\n", __func__, __LINE__);
	rev_type = lge_get_board_revno();
#if defined(CONFIG_LGE_PM_PCB_REV)
	if ((rev_type >= HW_REV_A) && slave_info->power_setting_array.size_a){
		...
		if (rev_type == HW_REV_A){
			size_down = slave_info->power_setting_array.size_down_0;
			if (!size_down || size_down > MAX_POWER_CONFIG){
				if (slave_info->power_setting_array.size_a){
					size_down = slave_info->power_setting_array.size_0;
					pr_info("%s: %d get power_down_setting_0 size\n", __func__, __LINE__);
				}
				else{
					size_down = slave_info->power_setting_array.size;
					pr_info("%s: %d get default power_down_setting size\n", __func__, __LINE__);
				}
			}
		}
		else
#endif
		{
			...
			if (!size_down || size_down > MAX_POWER_CONFIG){
				if (slave_info->power_setting_array.size_a){
					...
					pr_info("%s: %d get power_down_setting_a size\n", __func__, __LINE__);
				}
				else{
					size_down = slave_info->power_setting_array.size;
					pr_info("%s: %d get default power_down_setting size\n", __func__, __LINE__);
	...
	else if ((rev_type >= HW_REV_0) && slave_info->power_setting_array.size_0){
		/* DOWN */
		size_down = slave_info->power_setting_array.size_down_0;
		if (!size_down || size_down > MAX_POWER_CONFIG) {
			if (slave_info->power_setting_array.size_a) {
...
/* LGE_CHANGE_S, apply proper power setting, 2014-10-29, yt.jeon@lge.com */
	enum hw_rev_type rev_type = 0;
	pr_err("power_up_setting proper setting \n");
	rev_type = lge_get_board_revno();
	...
	if ((rev_type >= HW_REV_A) && slave_info->power_setting_array.size_a)
	{
		...
		pr_info("%s %d get power_setting_a size\n", __func__, __LINE__);
#if defined(CONFIG_MACH_MSM8909_M1_TMO_US) || defined(CONFIG_MACH_MSM8909_M1_MPCS_US)
		if (rev_type == HW_REV_A){
			size = slave_info->power_setting_array.size_0;
			pr_info("%s %d get power_setting_0 size\n", __func__, __LINE__);
		}
#endif
	}
	else if ((rev_type >= HW_REV_0) && slave_info->power_setting_array.size_0)
	{
		...
		pr_info("%s %d get default power_setting size\n", __func__, __LINE__);
	}
#endif
	if ((rev_type >= HW_REV_A) && slave_info->power_setting_array.size_a)
		{
...
/* LGE_CHANGE_S, apply proper power setting, 2014-10-29, yt.jeon@lge.com */
	{
#if defined(CONFIG_LGE_PM_PCB_REV)
		if ((slave_info->power_setting_array.power_setting_a!= NULL ) && (rev_type >= HW_REV_A)){
#if defined(CONFIG_MACH_MSM8909_M1_TMO_US) || defined(CONFIG_MACH_MSM8909_M1_MPCS_US)
			if (rev_type == HW_REV_A){
				if (copy_from_user(pu,
				(void *)slave_info->power_setting_array.power_setting_0,
				sizeof(*pu) * size)) {
				pr_err("failed: copy_from_user");
				kfree(pu);
				return -EFAULT;
				}
				pr_info("%s get power_setting_0\n", __func__);
			}
			else
#endif
			{
			...
			pr_info("%s get power_setting_a\n", __func__);
			}
		}
		else if ((slave_info->power_setting_array.power_setting_0 != NULL) && (rev_type >= HW_REV_0)){
			if (copy_from_user(pu,
				(void *)slave_info->power_setting_array.power_setting_0,
				sizeof(*pu) * size)) {
				pr_err("failed: copy_from_user");
			...
			pr_info("%s get default power_setting\n", __func__);
		}
#else
		if (copy_from_user(pu,
			(void *)slave_info->power_setting_array.power_setting,
...
#if defined(CONFIG_MACH_MSM8909_M1_SPR_US) || defined(CONFIG_MACH_MSM8909_M1DS_GLOBAL_COM) // change config to model, Original slave_addr is 0x50
		if(slave_info->sensor_id_info.sensor_id == 0x0553  // (if HI553 & slave_addr == 0x50) => HI553_FF
			&& slave_info->slave_addr == 0x50) {
			...
			if (rev_type <= HW_REV_A) {
#endif
				slave_info->slave_addr = 0x40;
				CDBG("SPR_US under HW_REV_A slave addr changed to %d", slave_info->slave_addr);
			}
		}
#endif // is SPR_US?
