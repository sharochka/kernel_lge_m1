	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_CLK:
			...
			break;
		case SENSOR_GPIO:
#if defined(CONFIG_MACH_MSM8909_M1V_VZW) || defined(CONFIG_MACH_MSM8909_M1V_LRA_US)
			if(power_setting->seq_val == SENSOR_GPIO_FL_EN){
				if(ctrl->vana == NULL){
					ctrl->vana = regulator_get(ctrl->dev, "rt5058-ldo3");
					if (IS_ERR(ctrl->vana)){
						pr_err("%s get failed rt5058-ldo3\n", __func__);
						return no_gpio;
					}
					regulator_set_voltage(ctrl->vana, 1200000, 1200000);
					pr_info("%s set rt5058-ldo3 1.2V\n",__func__);
				}
				rc = regulator_enable(ctrl->vana);
				break;
			}
			if(power_setting->seq_val == SENSOR_GPIO_FL_NOW){
				if(ctrl->ldo_io == NULL){
					ctrl->ldo_io = regulator_get(ctrl->dev, "rt5058-ldo1");
					if (IS_ERR(ctrl->ldo_io)){
						pr_err("%s get failed rt5058-ldo1\n", __func__);
						return no_gpio;
					}
					regulator_set_voltage(ctrl->ldo_io, 1800000, 1800000);
					pr_info("%s set rt5058-ldo1 1.8V\n",__func__);
				}
				rc = regulator_enable(ctrl->ldo_io);
				break;
			}
#endif
#if defined(CONFIG_MACH_MSM8909_E1Q_VZW) || defined(CONFIG_MACH_MSM8909_CLING_VZW)
			if(power_setting->seq_val == SENSOR_GPIO_CUSTOM1){
				if(ctrl->vdig_ldo3 == NULL){
					ctrl->vdig_ldo3 = regulator_get(ctrl->dev, "rt5058-ldo3");
					if (IS_ERR(ctrl->vdig_ldo3)){
						pr_err("%s get failed rt5058-ldo3\n", __func__);
						goto power_up_failed;
					}
					regulator_set_voltage(ctrl->vdig_ldo3, 1200000, 1200000);
					pr_info("%s set rt5058-ldo3 1.2V\n",__func__);
				}
				rc = regulator_enable(ctrl->vdig_ldo3);
				break;
			}
			else if(power_setting->seq_val == SENSOR_GPIO_CUSTOM2){
				if(ctrl->vdig_ldo2 == NULL){
					ctrl->vdig_ldo2 = regulator_get(ctrl->dev, "rt5058-ldo2");
					if (IS_ERR(ctrl->vdig_ldo2)){
						pr_err("%s get failed rt5058-ldo2\n", __func__);
						goto power_up_failed;
					}
					regulator_set_voltage(ctrl->vdig_ldo2, 1800000, 1800000);
					pr_info("%s set rt5058-ldo2 1.8V\n",__func__);
				}
				rc = regulator_enable(ctrl->vdig_ldo2);
				break;
			}
			else if(power_setting->seq_val == SENSOR_GPIO_VANA){
				if(ctrl->vana_ldo1 == NULL){
					ctrl->vana_ldo1 = regulator_get(ctrl->dev, "rt5058-ldo1");
					if (IS_ERR(ctrl->vana_ldo1)){
						pr_err("%s get failed rt5058-ldo1\n", __func__);
						goto power_up_failed;
					}
					regulator_set_voltage(ctrl->vana_ldo1, 2800000, 2800000);
					pr_info("%s set rt5058-ldo1 2.8V\n",__func__);
				}
				rc = regulator_enable(ctrl->vana_ldo1);
				break;
			}
#endif
			if (no_gpio) {
	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		...
		case SENSOR_GPIO:
#if defined(CONFIG_MACH_MSM8909_M1V_VZW) || defined(CONFIG_MACH_MSM8909_M1V_LRA_US)
			if(pd->seq_val == SENSOR_GPIO_FL_EN){
				if(ctrl->vana == NULL){
					ctrl->vana = regulator_get(ctrl->dev, "rt5058-ldo3");
					if (IS_ERR(ctrl->vana)){
						pr_err("%s get failed rt5058-ldo3\n", __func__);
				...
				ret = regulator_disable(ctrl->vana);
				break;
			}
			else if(pd->seq_val == SENSOR_GPIO_FL_NOW){
				if(ctrl->ldo_io == NULL){
					ctrl->ldo_io = regulator_get(ctrl->dev, "rt5058-ldo1");
					if (IS_ERR(ctrl->ldo_io)){
						pr_err("%s get failed rt5058-ldo1\n", __func__);
				...
				ret = regulator_disable(ctrl->ldo_io);
				break;
			}
#endif
