...
#if defined(HI553_LGIT_MODULE)
	unsigned short sl_add[] = {0xA1,0xA3,0xA5,0xA7,0xA9,0xAB,0xAD,0xAF};
#endif
...
#if defined(HI553_LGIT_MODULE)
			if(j < 8)
				e_ctrl->i2c_client.client->addr = sl_add[j];
#endif
...
static uint32_t msm_eeprom_checksum(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	//HI553 (LGIT) EEPROM MAP
	const int AWB_5100K_START_ADDR   = 0x0000;
	const int AWB_5100K_END_ADDR     = 0x0005;
	const int AWB_5100K_CHECKSUM_LSB = 0x0006;
	const int AWB_5100K_CHECKSUM_MSB = 0x0007;

	const int LSC_5100K_START_ADDR   = 0x000C;
	const int LSC_5100K_END_ADDR     = 0x037F;
	const int LSC_5100K_CHECKSUM_LSB = 0x0380;
	const int LSC_5100K_CHECKSUM_MSB = 0x0381;
	...
	if( CheckSum != DataSum ) {
        pr_err("%s EEPROM AWB CheckSum error for 5100K!\n", __func__);
    } else {
        pr_err("%s EEPROM AWB CheckSum for 5100K - OK\n", __func__);
	...
	if (CheckSum == 0) {
        ...
        pr_err("%s EEPROM LSC NOT Supported for 5100K!\n", __func__);
	}
	else if( CheckSum != DataSum ) {
		//LSC data exist, But CheckSum Failed!
        pr_err("%s EEPROM LSC CheckSum error for 5100K!\n", __func__);
    } else {
        //e_ctrl->is_supported |= 0x20; //LSC bit On
        pr_err("%s EEPROM LSC CheckSum for 5100K - OK\n", __func__);
		...
		switch(main_sensor_id){
			...
			case 0x06:
			case 0x07:
				pr_info("%s:main_sensor_id is LGIT \n", __func__);
				break;
			default:
				pr_err("%s:main_sensor_id is not LGIT (0x%x) retry address 0x399\n", __func__, main_sensor_id);
				main_sensor_id = e_ctrl->cal_data.mapdata[0x399];
				break;
		}
...
#else
/*LGE_CHANGE_S, add eeprom checksum, 2016-01-30 */
static uint32_t msm_eeprom_checksum(struct msm_eeprom_ctrl_t *e_ctrl)
{
	int32_t rc = -1;
	uint8_t eeprom_vendor = e_ctrl->cal_data.mapdata[MODULE_VENDOR_ID];
	pr_err("%s eeprom vendor addr = 0x%x, value = 0x%x\n", __func__,MODULE_VENDOR_ID, eeprom_vendor);
	main_sensor_id = e_ctrl->cal_data.mapdata[MODULE_VENDOR_ID];
...
#endif
/*LGE_CHANGE_E, add eeprom checksum, 2016-01-30 */
			 ...
			 const struct i2c_device_id *id)
{
	...
	rc = of_property_read_u32(client->dev.of_node, "qcom,sensor-position",
		&e_ctrl->position);
	CDBG("%s:%d sensor position %d, rc %d\n", __func__, __LINE__, e_ctrl->position, rc);
	if (rc < 0) {
		pr_err("%s:%d Default sensor position as 0\n", __func__, __LINE__);
		e_ctrl->position = 0;
	}
	...
	pr_err("%s AAT_Checksum %d\n", __func__, e_ctrl->AAT_Checksum);
	...
	CDBG("msm_eeprom_i2c_probe successfull");
#if defined(CONFIG_MACH_LGE)

	if(camera_vendor_id_class == NULL) {
