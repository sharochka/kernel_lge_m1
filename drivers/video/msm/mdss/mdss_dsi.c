				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->disp_en_p_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
	...
	ctrl_pdata->disp_en_n_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
	...
	ctrl_pdata->touch_en_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
		"qcom,platform-touch-en-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->touch_en_gpio))
		pr_info("%s: touch_en gpio not specified\n", __func__);
	...
	ctrl_pdata->dsv_toggle_enabled = of_property_read_bool(ctrl_pdev->dev.of_node,
