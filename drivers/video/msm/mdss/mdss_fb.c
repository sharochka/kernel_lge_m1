...
int lge_mdss_report_panel_dead(void)
{
	...
	if (mfd->panel_info != NULL &&
		mfd->panel_info->esd_check_enabled) {
	...
	backup_level = mfd->bl_level;
	pr_info("Panel has gone bad, sending uevent - %s\n", envp[0]);
	INIT_COMPLETION(mfd->unblank_done);
	kobject_uevent_env(&fbi->dev->kobj, KOBJ_CHANGE, envp);
	timeout = wait_for_completion_interruptible_timeout(&mfd->unblank_done, 5*HZ);
	if (!timeout)
...
static DEVICE_ATTR(panel_type, S_IRUGO, mdss_fb_get_panel_name, NULL); /* for miniOS */
...
static struct attribute *mdss_fb_attrs[] = {
	...
	&dev_attr_panel_type.attr, /* for miniOS */
...
#endif
			lge_get_laf_mode() == LGE_LAF_MODE_LAF || lge_get_boot_mode() == LGE_BOOT_MODE_CHARGERLOGO || lge_get_bootreason() == HIDDEN_FACTORY_RESET)
	{
