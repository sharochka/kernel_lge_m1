...
void vsync_notify_handler(void *arg)
{
	...
	if (oem_vsync_skip.enable_skip_vsync) {
	   ...
	   } else if (oem_vsync_skip.skip_value <= oem_vsync_skip.bucket) {
		struct mdp3_session_data *session = (struct mdp3_session_data *)arg;
		session->vsync_time = ktime_get();
		sysfs_notify_dirent(session->vsync_event_sd);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	switch (dfps) {
	case 51:
		ctrl_pdata->pclk_rate = 58750000;
		ctrl_pdata->byte_clk_rate = 88130000;
		pinfo->t_clk_post = 3;
		pinfo->t_clk_pre = 36;
		pd->timing[0] = 0xC2;
		pd->timing[1] = 0x2C;
		...
		break;
	case 52:
		ctrl_pdata->pclk_rate = 59900000;
		ctrl_pdata->byte_clk_rate = 89860000;
		pinfo->t_clk_post = 3;
		pinfo->t_clk_pre = 37;
		pd->timing[0] = 0xC5;
		pd->timing[1] = 0x2C;
		...
		break;
	case 53:
		ctrl_pdata->pclk_rate = 61060000;
		ctrl_pdata->byte_clk_rate = 91580000;
		pinfo->t_clk_post = 3;
		pinfo->t_clk_pre = 37;
		pd->timing[0] = 0xC6;
		pd->timing[1] = 0x2E;
		...
		break;
	case 54:
		ctrl_pdata->pclk_rate = 62210000;
		ctrl_pdata->byte_clk_rate = 93310000;
		pinfo->t_clk_post = 3;
		pinfo->t_clk_pre = 37;
		pd->timing[0] = 0xC4;
		pd->timing[1] = 0x2E;
		...
		break;
	case 55:
		ctrl_pdata->pclk_rate = 63360000;
		ctrl_pdata->byte_clk_rate = 95040000;
		pinfo->t_clk_post = 3;
		pinfo->t_clk_pre = 38;
		pd->timing[0] = 0xC9;
		pd->timing[1] = 0x30;
		...
		break;
	case 56:
		ctrl_pdata->pclk_rate = 64510000;
		ctrl_pdata->byte_clk_rate = 96770000;
		pinfo->t_clk_post = 3;
		pinfo->t_clk_pre = 38;
		pd->timing[0] = 0xCA;
		pd->timing[1] = 0x30;
		...
		break;
	case 57:
		ctrl_pdata->pclk_rate = 65660000;
		ctrl_pdata->byte_clk_rate = 98500000;
		pinfo->t_clk_post = 3;
		pinfo->t_clk_pre = 38;
		pd->timing[0] = 0xD2;
		pd->timing[1] = 0x32;
		...
		break;
	case 58:
		ctrl_pdata->pclk_rate = 66820000;
		ctrl_pdata->byte_clk_rate = 100220000;
		pinfo->t_clk_post = 3;
		pinfo->t_clk_pre = 40;
		pd->timing[0] = 0xD5;
		pd->timing[1] = 0x32;
		...
		break;
	case 59:
		ctrl_pdata->pclk_rate = 67970000;
		ctrl_pdata->byte_clk_rate = 101950000;
		pinfo->t_clk_post = 3;
		pinfo->t_clk_pre = 40;
		pd->timing[0] = 0xD6;
		pd->timing[1] = 0x32;
		...
		break;
	case 60:
		ctrl_pdata->pclk_rate = 69120000;
		ctrl_pdata->byte_clk_rate = 103680000;
		pinfo->t_clk_post = 3;
		pinfo->t_clk_pre = 41;
		pd->timing[0] = 0xDB;
		pd->timing[1] = 0x34;
	...
	case 61:
	case 62:
		ctrl_pdata->pclk_rate = 70270000;
		ctrl_pdata->byte_clk_rate = 105410000;
		pinfo->t_clk_post = 3;
		pinfo->t_clk_pre = 42;
		pd->timing[0] = 0xE1;
		pd->timing[1] = 0x34;
	...
	pdata->panel_info.mipi.frame_rate = dfps;
	ctrl_pdata->byte_clk_rate = ctrl_pdata->byte_clk_rate + (offset/8);
	ctrl_pdata->pclk_rate = ctrl_pdata->pclk_rate + (offset * lanes)/(bpp*8);
	...
	struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&dfps_lock);
	ret = snprintf(buf, PAGE_SIZE, "fps:%d,bitclk:%d\n",
		       pdata->panel_info.mipi.frame_rate, ctrl_pdata->byte_clk_rate*8);
	mutex_unlock(&dfps_lock);
					...
					void __user *argp)
{
	...
	case mdp_op_csc_cfg:
		...
		if (!mfd->panel_power_state) {
			pr_err("%s: before programming CSC blcok, fb already blanked. just return.\n", __func__);
			return -EPERM;
		}
    //LGE_UPDATE_E
