...
static void ts_mcu_sleep_control(int onoff);
#endif
...
#if defined(SENSOR_DETECT)
TouchDriverData *g_pDriverData = NULL;
#endif
...
static void report_finger(TouchDriverData *pDriverData, TouchReadData *pReadData)
{
	...
	if (palm_state) {
		TOUCH_LOG("Pressed touch is cancel.\n");
		palm_state = 0;
...
switch(readData.type) {
...
#endif
	case DATA_HOVER:
		if (pDriverData->currState == STATE_NORMAL_HOVER || pDriverData->currState == STATE_HOVER) {			/* report hover state */
	...
	default:
		if (palm_changed) {
			TOUCH_LOG("Unknown event from device ( event = %d, STATE = %d )\n", readData.type, pDriverData->currState);
		}
        break;	
	}

	palm_changed = palm_state;
...
static void UpdateLpwgSetting ( LpwgSetting *pLpwgSetting, LpwgCmd lpwgCmd, int *param )
{
	...
	switch (lpwgCmd) {
		case LPWG_CMD_MODE:
			TOUCH_LOG("LPWG_CMD_MODE\n");
			pLpwgSetting->mode = param[0];
			sprintf(str, "%s", "MODE");
			break;

		case LPWG_CMD_LCD_PIXEL_SIZE:
			TOUCH_LOG("LPWG_CMD_LCD_PIXEL_SIZE\n");
		...
		case LPWG_CMD_ACTIVE_TOUCH_AREA:
			TOUCH_LOG("LPWG_CMD_ACTIVE_TOUCH_AREA\n");
...
static TouchState DecideNextDriverState( LpwgSetting *pLpwgSetting )
{
	...
	if (pLpwgSetting->lcdState == 1) {
		if (pLpwgSetting->callState == 2) {
			TOUCH_LOG("next = STATE_NORMAL_HOVER\n");
			nextState = STATE_NORMAL_HOVER;
		} else {
			TOUCH_LOG("next = STATE_NORMAL\n");
			nextState = STATE_NORMAL;
		}
	} else {
		if (pLpwgSetting->callState == 2) {
			TOUCH_LOG("next = STATE_HOVER\n");
			nextState = STATE_HOVER;
		} else {
			if( pLpwgSetting->mode == 0 ) {
				TOUCH_LOG("next = STATE_OFF\n");
				nextState = STATE_OFF;
			} else if( pLpwgSetting->mode == 1 ) {
				TOUCH_LOG("next = STATE_KNOCK_ON_ONLY\n");
				nextState = STATE_KNOCK_ON_ONLY;
			} else if( pLpwgSetting->mode == 2 ) {
				TOUCH_LOG("next = STATE_KNOCK_ON_CODE\n");
...
static ssize_t show_version(struct i2c_client *client, char *buf)
{
	...
	if (lge_get_factory_boot()){
		pDeviceSpecificFunc->GetProduct_id(client,productName);
		ret += sprintf(buf+ret, "FW Version: [%s]\n",productName);
		TOUCH_LOG("[%s] product name = %s",__func__,productName);
	}
	ret += sprintf(buf+ret,	"FW Version: %X.%02X\n",	pDriverData->icFwInfo.isOfficial, pDriverData->icFwInfo.version);
...
* To Write : echo write $regHigh $regLow $data > ic_rw
* To Read : echo read $regHigh $regLow > ic_rw ==> cat > ic_rw
****************************************************************************/
static int regAddr1=0, regAddr2=0, readCount=0;
static ssize_t show_ic_rw(struct i2c_client *client, char *buf)
{
	...
	for(i=0; i<readCount; i++) {
		result = pDeviceSpecificFunc->AccessRegister(client, READ_IC_REG, regAddr1, regAddr2, &readValue);
		if(result == TOUCH_FAIL) {
			TOUCH_ERR("failed to read register ( reg = %d )\n", ((regAddr1*100) + regAddr2));
			break;
		}
		ret += sprintf(buf+ret, "0x%02X%X=%d\n", regAddr1, regAddr2, readValue);
		regAddr2++;
	}
...
static ssize_t store_ic_rw(struct i2c_client *client, const char *buf, size_t count)
{
	...
	sscanf(buf, "%s %x %x %d", cmd, &reg11, &reg21, &data);
	...
	TOUCH_LOG("HEX reg11 = %x, reg21 = %x\n",reg11,reg21);
	TOUCH_LOG("DEC reg11 = %d, reg21 = %d\n",reg11,reg21);
	TOUCH_LOG("HEX reg12 = %x, reg22 = %x\n",reg12,reg22);
	TOUCH_LOG("DEC reg12 = %d, reg22 = %d\n",reg12,reg22);

	mutex_lock(pMutexTouch);

	if(strcmp(cmd, "write") == 0) {
		ret = pDeviceSpecificFunc->AccessRegister(client, WRITE_IC_REG, reg11, reg21, &data);
		if (ret == TOUCH_FAIL) {
			/* TOUCH_ERR("failed to write register (reg = %d%d, data =%d)\n", reg12, reg22, data); */
			TOUCH_ERR("failed to write register (reg = %x%x, data =%d)\n", reg11, reg21, data );
		} else {
			/* TOUCH_DBG("Success to write register (reg = %d%d, data =%d)\n", reg11, reg21, data); */
			TOUCH_DBG("Success to write register (reg = %x%x, data =%d)\n", reg11, reg21, data );
		}
	} else {
		/* TOUCH_DBG("Ready to read register ( reg = %x )\n", ((reg12*100)+reg22)); */
		TOUCH_DBG("Ready to read register (reg = %x%x)\n", reg11,reg21);
		regAddr1 = (int)reg11;
		regAddr2 = (int)reg21;
...
{
	int ret = 0;
	int rawData_read_result = TOUCH_SUCCESS;

	TOUCH_FUNC();
	ret = snprintf(buf, PAGE_SIZE, "========RESULT=======\n");
	rawData_read_result = pDeviceSpecificFunc->Getraw_data_result(client,buf);

	if (rawData_read_result == TOUCH_SUCCESS) {
		ret += sprintf(buf,"LPWG RawData : Pass\n");
	} else {
		ret += sprintf(buf,"LPWG RawData : Fail\n");
	}
	return ret;
}
...
static LGE_TOUCH_ATTR(mfts_lpwg, S_IRUGO | S_IWUSR, show_mfts_lpwg_test, NULL);
	...
	if(event == FB_EARLY_EVENT_BLANK) {
		if (*blank == FB_BLANK_POWERDOWN) {
			...
			if (pDriverData->enable_sensor_interlock) {
				pDeviceSpecificFunc->mcu_sleep_control(pDriverData->client);
			}
			*/
			#endif
			/* if (pDriverData->bootMode == BOOT_MINIOS) { */
			if (lge_get_factory_boot()) {
				TOUCH_LOG("Mini OS operated\n");
				release_all_touch_event(pDriverData);
				pDriverData->lpwgSetting.lcdState = 0;

				/* LPWG mode ( Bit field ) : 0 = disable, 1 = knock-on, 2 = knock-code, 4 = Signature(Not Used) */
				pDriverData->lpwgSetting.mode = 1;
				/* 1 = Cover Close, 0 = Open*/
				pDriverData->lpwgSetting.coverState = 0;
				/* 0 = Call End, 1 = Ringing, 2 = Call Received */
				pDriverData->lpwgSetting.coverState = 0;
				pDeviceSpecificFunc->SetLpwwgDebug_enable(pDriverData->client,1);
			...
			#if defined (TOUCH_DEVICE_MIT200_PROC4)
			if (lge_get_factory_boot()) {
				pDeviceSpecificFunc->SetLpwwgDebug_enable(pDriverData->client,0);
			}

			#if defined(SENSOR_DETECT)
			pDriverData->sensor_value = 0; /* sensoroff when turn on the LCD */
			if(pDriverData->enable_sensor_interlock) {
				pDeviceSpecificFunc->mcu_sleep_control(pDriverData->client);
			}
			#endif
...
{
	int ret = 0;
	int retires = MAX_FW_UPGRADE_RETRY;
	...
	if (pDriverData->bootMode != BOOT_OFF_CHARGING) {
		...
		TOUCH_LOG("[%s] pDriverData->bootMode != BOOT_OFF_CHARGING \n",__func__);
		TOUCH_LOG("TouchIrqHandler will be Registered");
	...
	{
		if ((pDriverData->binFwInfo.version != pDriverData->icFwInfo.version) || retires <= 0)
		{
			pDriverData->useDefaultFirmware = TOUCH_TRUE;
			queue_delayed_work(touch_wq, &pDriverData->work_upgrade, 0);
		} else {
	...
	pDeviceSpecificFunc->SetLpwwgDebug_enable(client,0);

#if defined (SENSOR_DETECT)
	g_pDriverData = pDriverData;
	pDriverData->sensor_value = 0;
	pDriverData->enable_sensor_interlock = 1;
	TOUCH_LOG("sensor_value set to %d, enable_sensor_interlock set to %d\n",pDriverData->sensor_value,pDriverData->enable_sensor_interlock);
	rpr0521_register_proximity_change_callback(ts_mcu_sleep_control);
#endif
	return TOUCH_SUCCESS;
...
static void ts_mcu_sleep_control(int onoff)
{
	TOUCH_LOG("apds interrupt onoff : %d\n",onoff);
	g_pDriverData->sensor_value = onoff;
/*
	if (g_pDriverData->enable_sensor_interlock) {
		if (onoff){//near : 1, far : 0
			if (g_pDriverData->lpwgSetting.lcdState == 0) {
				pDeviceSpecificFunc->mcu_sleep_control(g_pDriverData->client);
			} else {
				TOUCH_LOG("[%s] Don't adjust MCU sleep contol\n",__func__);
			}
...
static int __init touch_init(void)
{
	...
	TOUCH_ERR("######## [%s] ##########\n",(char*)lge_touch_driver.driver.of_match_table->compatible);
