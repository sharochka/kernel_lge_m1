...
{
#ifdef CONFIG_LGE_USE_HARD_RESET
...
#ifdef CONFIG_LGE_USE_HARD_RESET
        if (cmd != NULL) {
            if ((cmd[0] == '\0') || !strncmp(cmd, "Restarted by power key", 22) || !strncmp(cmd, "bootchart", 9) || !strncmp(cmd, "eMMCTest", 8)) {
                pr_notice("suker : reboot cmd = %s --- try HARD RESET!!\n", cmd);
                is_warm_reset = false;
            }
        } else {
            pr_notice("suker : reboot cmd is NULL --- try HARD RESET!!\n");
            is_warm_reset = false;
        }
        if (is_warm_reset || get_dload_mode() || in_panic || (restart_mode == RESTART_DLOAD)) {
            pr_notice("suker : qpnp WARM BOOT\n");
            qpnp_pon_system_pwr_off(PON_POWER_OFF_WARM_RESET);
        }
        else {
            pr_notice("suker : qpnp COLD BOOT\n");
