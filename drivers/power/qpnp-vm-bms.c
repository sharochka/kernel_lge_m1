...
#endif
#ifdef CONFIG_LGE_PM_VM_BMS_BATTERY_PRESENT_WORKAROUND
...
#ifdef CONFIG_LGE_PM_BATTERY_SOC_RESCALING
				if(chip->last_soc < SOC_RESCALING_FACTOR)
#endif
...
#ifdef CONFIG_LGE_PM_BATTERY_SOC_RESCALING
#if (defined (CONFIG_LGE_PM_BATTERY_CAPACITY_1940mAh) || \
     defined (CONFIG_LGE_PM_BATTERY_CAPACITY_2125mAh))
#define SOC_BUF_SIZE 30
#else
...
#ifdef CONFIG_LGE_PM_BATTERY_SOC_RESCALING
	pr_debug("charging=%d last_soc=%d last_soc_unbound=%d last_ocv=%d\n",
...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	switch ( battery_id ){
#ifdef CONFIG_LGE_PM_BATTERY_CAPACITY_1940mAh
	case BATT_ID_SW3800_VC0 :
	case BATT_ID_RA4301_VC1 :
		battery_profile = "lgc,battery-data";
		pr_info("[BATTERY PROFILE] Using battery profile - "\
				"LGChem_1940mAh for id(%lld)\n", battery_id);
		break;
	case BATT_ID_RA4301_VC0 :
		battery_profile = "byd,battery-data";
		pr_info("[BATTERY PROFILE] Using battery profile - "\
				"BYD_1940mAh for id(%lld)\n", battery_id);
		break;
	...
	default :
		...
		pr_info("[BATTERY PROFILE] No battery ID matching\n"\
				...
				"LGChem_1940mAh for id(%lld)\n", battery_id);
		break;
#elif defined(CONFIG_LGE_PM_BATTERY_CAPACITY_2125mAh)
	case BATT_ID_SW3800_VC0 :
	case BATT_ID_RA4301_VC1 :
		battery_profile = "lgc,battery-data";
		pr_info("[BATTERY PROFILE] Using battery profile - "\
				"LGChem_2125mAh for id(%lld)\n", battery_id);
		break;
	case BATT_ID_SW3800_VC1 :
	case BATT_ID_RA4301_VC0 :
		battery_profile = "tocad,battery-data";
		pr_info("[BATTERY PROFILE] Using battery profile - "\
				"Tocad_2125mAh for id(%lld)\n", battery_id);
		break;
	case BATT_ID_ISL6296_C :
	case BATT_ID_ISL6296_L :
	case BATT_ID_DS2704_L :
	...
	if (batt_id == BATT_ID_ISL6296_L)
		...
		node = of_find_node_by_name(chip->spmi->dev.of_node,
						"lg,battery-data");
	else if (batt_id == BATT_ID_SW3800_VC0)
		node = of_find_node_by_name(chip->spmi->dev.of_node,
						"lgc,battery-data");
	else
		node = of_find_node_by_name(chip->spmi->dev.of_node,
