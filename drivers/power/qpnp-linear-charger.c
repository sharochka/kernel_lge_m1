...
#elif defined (CONFIG_LGE_PM_LGE_POWER_CLASS_CHARGING_CONTROLLER)
static void lgcc_charger_reginfo(void);
#define BATT_TEMP_OVERHEAT 57
#define BATT_TEMP_COLD (-10)
static int lgcc_charging_current;
...
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_CHARGING_CONTROLLER
int get_btm_state(struct qpnp_lbc_chip *chip)
{
	return chip->btm_state;
}
int get_pseudo_ui(struct qpnp_lbc_chip *chip)
{
	return chip->pseudo_ui;

}
int lgcc_is_probed(struct qpnp_lbc_chip *chip)
{
	chip->lge_cc_lpc = lge_power_get_by_name("cc");
	if (!chip->lge_cc_lpc) {
		pr_err("%s : lgcc is not probed yet!!\n", __func__);
		return 0;
	}
	else {
		pr_debug("%s : lgcc is probed\n", __func__);
		return 1;
	}
...
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_CABLE_DETECT
/* cable detect code will be remove in charger code. */
#else
#define LT_CABLE_56K		6
#define LT_CABLE_130K		7
#define LT_CABLE_910K		11
#endif
#ifndef CONFIG_LGE_PM_LGE_POWER_CLASS_CABLE_DETECT
...
static bool is_factory_cable(void)
{
	...
	if (qpnp_chg == NULL)
		...
		return qpnp_chg->is_factory_cable;
#else
...
/* cable detect code will be remove in charger code. */
	...
	if (usb_current_max_enabled) {
		if (chip->usb_present && chg_current > QPNP_LBC_IBATMAX_MIN) {
			pr_err("USB Current Max set charging current to %d(mA)\n", USB_CURRENT_MAX);
...
static int get_prop_batt_health(struct qpnp_lbc_chip *chip)
{
	...
	if(chip->usb_present && (lgcc_is_probed(chip) == 1)){
		if (get_btm_state(chip) == BTM_HEALTH_OVERHEAT)
			return POWER_SUPPLY_HEALTH_OVERHEAT;
		if (get_btm_state(chip) == BTM_HEALTH_COLD)
			return POWER_SUPPLY_HEALTH_COLD;
		else
			return POWER_SUPPLY_HEALTH_GOOD;
	} else {
...
#elif defined (CONFIG_LGE_PM_LGE_POWER_CLASS_CHARGING_CONTROLLER)
	if (qpnp_lbc_is_usb_chg_plugged_in(chip)
			&& (lgcc_is_probed(chip) == 1) && get_pseudo_ui(chip)){
#ifdef CONFIG_LGE_PM
		if (!wake_lock_active(&chip->chg_wake_lock))
			wake_lock(&chip->chg_wake_lock);
		spin_unlock_irqrestore(&chip->chg_sts_chk, flags);
...
static void qpnp_batt_external_power_changed(struct power_supply *psy)
{
	...
	if (qpnp_lbc_is_usb_chg_plugged_in(chip)) {
		...
		current_ma = lge_val.intval / 1000;
		pr_err("current_ma cd : %d\n",current_ma);
#else
...
#ifdef CONFIG_LGE_PM_LG_POWER_CORE
static void
qpnp_batt_external_lge_power_changed(struct power_supply *psy)
{
	struct qpnp_lbc_chip *chip
			= container_of(psy, struct qpnp_lbc_chip,
								batt_psy);
	int rc = 0;
	union lge_power_propval lge_val = {0,};
	unsigned long flags;
	...
	if (!chip->lge_cc_lpc) {
		...
		lgcc_set_ibat_current(lgcc_ibatt_current);
...
skip_current_config2:
	...
	if (!chip->lge_vzw_lpc) {
		...
		rc = qpnp_lbc_charger_enable(chip, VZW_REQ, lge_val.intval);
	}
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		{
			rc = qpnp_lbc_charger_enable(chip, INVALID_BATTERY,
	...
	case POWER_SUPPLY_PROP_USB_CURRENT_MAX:
		...
		qpnp_lbc_ibatmax_set(qpnp_chg, qpnp_chg->usb_psy_ma);
		break;
#endif
	default:
		return -EINVAL;
...
static int qpnp_lbc_main_probe(struct spmi_device *spmi)
{
	...
	if (chip->bat_if_base) {
		...
		chip->batt_psy.external_lge_power_changed =
			qpnp_batt_external_lge_power_changed;
#endif
