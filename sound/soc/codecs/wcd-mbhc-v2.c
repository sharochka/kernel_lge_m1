				...
				struct snd_soc_jack *jack, int status, int mask)
{
	...
	/* there is no code to handle SW_LINEOUT_INSERT in LGE framework */
				...
				const enum wcd_mbhc_cs_mb_en_flag cs_mb_en)
{
	...
	case WCD_MBHC_EN_CS:
       ...
       if(lge_mbhc_check_factory_boot_mode()){
            pr_info("%s: pull up enable in FSM", __func__);
            /* enable FSM */
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		if (mbhc->mbhc_cb->set_switch_hptype)
			mbhc->mbhc_cb->set_switch_hptype(mbhc->codec, 0, 0, 0);
#endif
...
#ifndef CONFIG_AUDIO_LGE_CODE_REMOVAL
		if (mbhc->mbhc_cb->set_switch_hptype)
			mbhc->mbhc_cb->set_switch_hptype(mbhc->codec, mbhc->zl, jack_type, 1);
#endif
		wcd_mbhc_jack_report(mbhc, &mbhc->headset_jack,
...
#if CONFIG_MACH_LGE	/* LGE UPDATE : Return false to avoid malfunction of several AUX cables */
...
#if CONFIG_MACH_LGE 	/* LGE UPDATE : Add to detect apple headset fast & detect AUX cable as 4 pin headset */
...
static int wcd_mbhc_get_button_mask(struct wcd_mbhc *mbhc)
{
	...
	case 3:
		...
		break;
#ifdef CONFIG_AUDIO_LGE_CODE_REMOVAL
	case 4:
		mask = SND_JACK_BTN_4;
		break;
	case 5:
