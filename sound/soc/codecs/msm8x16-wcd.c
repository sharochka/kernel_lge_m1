...
static const struct msm8x16_wcd_reg_mask_val
	 ...
	 */
#ifndef CONFIG_AUDIO_LGE_CODE_REMOVAL
	/* Note for LGE PM8916 headset mute issue : case 01942111
	* 0x191 RX_COM_OCP_CTL = 0xDF is our default setting for the OCP.
	* LGE should use this to avoid issue.
	* Every time the current is over the threshold,
	* OCP immediately shut down PA to protect the circuit
	* N_CONN_ATTEMPT sets the number of PA re-enable times
	* before generating an OCP interrupt,
	* if over current condition no longer exist after some wait period,
	* HPH PA function will resume.
	* Once OCP interrupt generated, SW will need to service it and
	* reset HPH PA. HPH PA function will not self resume.
	*/
...
#ifdef CONFIG_MSM8909_M209
	{MSM8X16_WCD_A_ANALOG_MICB_1_INT_RBIAS, 0x60, 0x20},
#endif
};
...
#ifdef CONFIG_AUDIO_LGE_USE_HSD_3TYPE
			if (zl > 4000) {
...
#if 1	/* [LGE Change] Disable L-impedance measuring during detecting */
