...
DECLARE_EVENT_CLASS(msm_smem_buffer_ion_ops,
	...
	TP_PROTO(char *buffer_op, u32 buffer_type, u32 heap_id_mask,
		size_t size, u32 align, u32 flags, int map_kernel),

	TP_ARGS(buffer_op, buffer_type, heap_id_mask, size, align,
		flags, map_kernel),

	TP_STRUCT__entry(
		__field(char *, buffer_op)
		__field(u32, buffer_type)
		__field(u32, heap_id_mask)
		__field(u32, size)
		__field(u32, align)
		__field(u32, flags)
	...
	TP_printk(
		"%s, buffer_type : 0x%x, heap_id_mask : 0x%x, size : 0x%x, align : 0x%x, flags : 0x%x, map_kernel : %d",
		__entry->buffer_op,
		__entry->buffer_type,
